...
describe ApplicationsController do
  ...
  before(:each) do
    # force the use of the southwark theme
    ThemeChooser.stub(:themer_from_request).and_return(SouthwarkTheme.new)
  end

  context "overridden search action" do
    it "should assign the list of categories" do
      get :search
      expect(assigns(:categories)).to eq Configuration::THEME_SOUTHWARK_CATEGORIES
    end

    it "should assign a list of categories as json for the javascript" do
      get :search
      expect(assigns(:categories_json)).to eq Configuration::THEME_SOUTHWARK_CATEGORIES.as_json
    end
    ...
    context 'matching a MapIt authority' do
      before :each do
        ...
        SouthwarkSearch.should_receive(:new)
    ...
    context "deciding whether to show results" do
      ...
      context "on the initial search page" do
        it "should default to not showing any results" do
          SouthwarkSearch.should_not_receive(:new)
          get :search
        ...
        it "should not show the results view if there was an error with the postcode" do
          stub_search = stub()
          SouthwarkSearch.should_receive(:new).and_return(stub_search)
    ...
    context "deciding whether to show the map" do
      ...
      before do
        SouthwarkSearch.stub(:new).and_return(stub_search)
      end
