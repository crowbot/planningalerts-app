...
feature "Custom southwark search pages" do
  before(:each) do
    # mock search to be overridden for specific scenarios
    @mock_search = mock_model(SouthwarkSearch)
  ...
  scenario "Invalid postcode input in the search widget" do
    VCR.use_cassette('southwark_theme_valid_looking_invalid_postcode', :record => :once, :allow_playback_repeats => true) do
      visit search_applications_url(:host => "southwark.127.0.0.1.xip.io:3000", :results => true, :status => "all", :location => "SW99 0AA")
    end
  ...
  scenario "No matching results" do
    SouthwarkSearch.should_receive(:new).and_return(@mock_search)
    mock_results = mock_search_results(0)
    @mock_search.stub(:perform_search).and_return(mock_results)

    visit search_applications_url(:host => "southwark.127.0.0.1.xip.io:3000", :results => true, :status => "all", :location => "GU14 7ST")
  ...
  scenario "4 matching results with a matching MapIt authority (as a list)" do
    ...
    visit search_applications_url(:host => 'southwark.127.0.0.1.xip.io:3000', :results => true, :status => 'all', :location => 'GU14 7ST', :display => 'list')
  ...
  scenario "results filtered on status" do
    ...
    visit search_applications_url(:host => 'southwark.127.0.0.1.xip.io:3000', :results => true, :location => 'GU14 7ST', :display => 'list', :status => 'pending')
    expect(page).to have_no_selector('div#sidebar-stats')
  end
