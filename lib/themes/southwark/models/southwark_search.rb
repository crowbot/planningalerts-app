class SouthwarkSearch < ApplicationSearch
  ...
  def search_range
    distance_in_miles = ::Configuration::THEME_SOUTHWARK_SEARCH_RADIUS
  ...
  def calculate_stats(results)
    ...
    approved_count = results.facets[:status][Configuration::THEME_SOUTHWARK_STATUSES['approved']].to_i
    if approved_count > 0
    ...
    refused_count = results.facets[:status][Configuration::THEME_SOUTHWARK_STATUSES['refused']].to_i
    if refused_count > 0
    ...
    current_count = results.facets[:status][Configuration::THEME_SOUTHWARK_STATUSES['pending']].to_i
    if current_count > 0
      stats[:percentage_pending] = (current_count.to_f / results.total_entries * 100).round
    else
      stats[:percentage_pending] = 0
    end
